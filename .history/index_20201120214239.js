...
function getSetLength(rows) {
  ...
  const lastRowStart = setLength - rows;
  const lastRowEnd = setLength - 1;

  const lastRow = set.splice(lastRowStart, rows);

  return lastRow.reduce((prev, curr) => prev+curr);

}

console.log(getSetLength(5));
// row 1
//  1
